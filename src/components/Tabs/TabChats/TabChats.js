...
const TabChats = (props) => {
    ...
    const loadUsers = () => {
        database.ref(`${platform}/users`).get().then(x=> x.val()).then(x => setUsers(prev => x));
    }
    ...
    const getRoute = () => {
        ...
        if(selectedChat) res.push(
            {
                name: chats[selectedChat].username,
                resetFunc: () => {
                    setSelectedChat("");
    ...
    useEffect(() => {
        if(platform)
            loadUsers();
        else setUsers(prev => null);
    },[platform]);
    //useEffect(()=> console.log('reload'));
    useEffect(() => {
        if(selectedUser)
            setChats(prev => users[selectedUser].chats || {});
        else setChats(prev => null);
    },[selectedUser]);
    ...
    return (
        <React.Fragment>
            <ChatsBreadcrumb route={getRoute()}/>
            {pageContent}
        </React.Fragment>
